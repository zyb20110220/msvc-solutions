...
export interface VcxprojTree {
  ...
  // nested filters structure: { name: string, children?: Record<string, FilterNode>, files?: string[] }
  filters?: FilterNode[];
}
...
export function parseVcxproj(filePath: string): VcxprojTree {
  ...
  const filtersMap: { [k: string]: string[] } = {};
  ...
  for (const g of groups) {
    ...
    if (g.ClInclude) {
      ...
      for (const it of arr) {
        ...
        if (inc) headers.push(path.normalize(path.resolve(path.dirname(filePath), inc)));
      }
    }
    if (g.ClCompile) {
      ...
      for (const it of arr) {
        const inc = it['@_Include'] || it['Include'];
        if (inc) sources.push(path.normalize(path.resolve(path.dirname(filePath), inc)));
      }
    }
    if (g.ResourceCompile || g.Resource) {
      ...
      for (const it of arr) {
        const inc = it['@_Include'] || it['Include'];
        if (inc) resources.push(path.normalize(path.resolve(path.dirname(filePath), inc)));
      }
    }
    if (g.Reference) {
    ...
    if (g.ProjectReference) {
      ...
      for (const it of arr) {
        const inc = it['@_Include'] || it['Include'];
        if (inc) projectRefs.push(path.normalize(path.resolve(path.dirname(filePath), inc)));
      }
    }
  }
  ...
  try {
    ...
    if (fs.existsSync(filtersPath)) {
      ...
      const pf = new XMLParser({ ignoreAttributes: false });
  ...
  function buildTree(map: { [k: string]: string[] }): FilterNode[] {
    ...
    function toNodes(obj: any): FilterNode[] {
      return Object.keys(obj).map(k => {
        const entry = obj[k];
        const node: FilterNode = { name: k };
        if (entry.__files && entry.__files.length) node.files = entry.__files;
